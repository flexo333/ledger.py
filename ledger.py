...
def justify_columns(seq_of_seq_of_strings, justification):
    ...
    # FIXME: *just_col* need rewriting to remove duplicate code
...
def rjust_column(seq_of_seq_of_strings, column):
    ...
    # FIXME: ljust_col*, rjust_col* need rewriting to remove duplicate code
    if (len(seq_of_seq_of_strings) == 0):
        return seq_of_seq_of_strings
    # else
    #CHANGEME: Make copy where inner sequences are lists, so we can modify them
...
# XXX Units / Currencies

# FIXME: This code isn't good for anything besides AUD at the moment.
...
def root_account_name(account_string):
    ...
    Convert to upper case and plural. Replace Revenue with income"""
    ...
    return REG_ACCOUNT_NAMES.get(root, root)
...
def ensure_date_sorted(transactions):
    ...
    for transaction in transactions[1:]:
        # FIXME: Should throw an exception rather than exiting completely.
...
def verify_balances(transactions, verifications, verbose, exit_on_failure):
    ...
    for transaction in transactions:
        while verifications and transaction['date'] > verifications[0]['date']:
...
def extract_accounts(transactions):
    # FIXME: I believe this is useless and not used
...
def single_unit_report_helper(accounts_dict, account_names=None, prefix="", indent=0):
    ...
    result = []
    if account_names:
        # FIXME: Fix this, so it does something useful....
        for account_name in account_names:
            account = find_account(account_name, accounts_dict)
...
def validate_one_date_or_two(as_at_date, first_date, last_date):
    if as_at_date:
        ...
        if first_date:
            sys.stderr.write("Error: first-date: %s specified in"
                             " addition to as-at-date '%s'.\n"
                             "Exiting." % (first_date, as_at_date))
            sys.exit(-1)
        if last_date:
            sys.stderr.write("Error: last-date: %s specified in"
                             " addition to as-at-date '%s'.\n"
    ...
    if (first_date):
        ...
        if not last_date:
            sys.stderr.write("Error: first-date: %s specified, "
                             "but last-date unspecified.\n"
                             "Exiting." % (first_date))
            sys.exit(-1)
    if last_date:
        if not first_date:
            sys.stderr.write("Error: last-date: %s specified, "
                             "but first-date unspecified.\n"
                             "Exiting." % (last_date))
            sys.exit(-1)
...
def write_excel_report(transactions, dates, output_filename):
    TXN_COLOUR = 7
    # FIXME: Should allow list of accounts to be named.
    dates = sorted(dates)
    num_dates = len(dates)
    # Check we at least one date
    if num_dates < 1:
        # FIXME: Should print final balances if no dates specified
        # instead of dieing...
    ...
    for idx, date in enumerate(dates):
        ws.write(1, idx, date, column_heading_style)
    # Write date headings for balances
    for idx, date in enumerate(dates, start=1):
        ws.write(1, idx + num_dates, date, column_heading_style)
    # Freeze Panes
...
def main():
    ...
    if (args.print_balances is not None):
